...
import { ImagePlus, Loader2, MessageCircle, RotateCcw, Save, Send, ShoppingCart, Sparkles, Star, Wand2 } from "lucide-react";

import { ReferenceImageSelector } from "@/components/editor/reference-image-selector";
...
function getActionText(action: string | null) {
  ...
  if (action === "refine") return "正在基于当前图进行AI微调，请稍候...";
  return "";
}
...
export function EditorWorkspace({ project: initialProject }: EditorWorkspaceProps) {
  ...
  const runGeneration = async (kind: "generate" | "regenerate") => {
    ...
    try {
      // 自定义模式下，生成前先自动保存用户编辑的内容
      if (isCustomMode) {
        try {
          await fetch(`/api/projects/${project.id}/sections/${selectedSection.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              type: String(selectedSection.type).toLowerCase(),
      ...
      const response = await fetch(`/api/projects/${project.id}/sections/${selectedSection.id}/${kind}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ referenceAssetIds: checkedReferences, customMode: isCustomMode }),
      });
      const payload = await response.json();
      if (!payload.success) {
  ...
  const runImageEdit = async (editMode: "repaint" | "enhance") => {
    ...
    try {
      ...
      const response = await fetch(`/api/projects/${project.id}/sections/${selectedSection.id}/edit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ referenceAssetIds: checkedReferences, editMode, customMode: isCustomMode }),
      });
      const payload = await response.json();
      if (!payload.success) {
  ...
  const runRefine = async () => {
    ...
    setRunningAction("refine");

    try {
      // 自定义模式下，微调前先自动保存用户修改的内容
      if (isCustomMode) {
        try {
          await fetch(`/api/projects/${project.id}/sections/${selectedSection.id}`, {
            method: "PATCH",
      ...
      const response = await fetch(`/api/projects/${project.id}/sections/${selectedSection.id}/refine`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const payload = await response.json();
      if (!payload.success) {
        toast.error(payload.error?.message ?? "基于当前图微调失败");
        return;
      }
      if (payload.data?.generationMode === "svg_fallback") {
        toast.warning("当前 Provider 没有可用真实图片端点，本次结果为 SVG 兜底预览。");
      } else {
        toast.success("已基于当前图完成AI微调，并自动保存新版本");
      }
          ...
          {!selectedSection ? (
            ...
            <>
              ...
              <ReferenceImageSelector
                selectedIds={checkedReferences}
                onSelect={setCheckedReferences}
              ...
              <div className="space-y-3 rounded-2xl border border-border p-3">
                ...
                <div className="rounded-2xl border border-border bg-gradient-to-br from-violet-50/50 to-purple-50/50 p-3 dark:from-violet-950/20 dark:to-purple-950/20">
                  <div className="flex items-center gap-2">
                    <Wand2 className="h-4 w-4 text-violet-600" />
                    <p className="text-sm font-medium">基于当前图AI微调</p>
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">输入你的微调想法，AI会根据当前图片和你的描述进行智能调整。</p>
                  <div className="mt-3 space-y-3">
                    <Textarea
                      placeholder="例如：把背景换成暖色调、增加一些光影效果、让产品更突出..."
                    ...
                    <Button
                      onClick={runRefine}
                      disabled={!hasGeneratedImage || Boolean(runningAction) || !refineInstruction.trim()}
                      className="w-full gap-2 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700"
                    >
                      {runningAction === "refine" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                      {runningAction === "refine" ? "AI正在微调中..." : "发送微调指令"}
                    </Button>
                  </div>
                </div>
