...
import { assetTypeLabels, platformLabels, platformOptions, styleCategories, styleLabels, styleOptions } from "@/types/domain";
...
function PreviewGrid(props: {
  ...
  const urlsRef = useRef<Array<{ url: string; key: string }> | null>(null);
  const urls = useMemo(() => {
    if (urlsRef.current) {
      urlsRef.current.forEach((item) => URL.revokeObjectURL(item.url));
    }
    urlsRef.current = props.files.map((file, index) => ({
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      {urls.map((item, index) => {
        ...
        return (
          <div key={item.key} className="overflow-hidden rounded-2xl border border-border bg-white">
            <div className="aspect-square bg-slate-100">
              <img src={item.url} alt={file.name} className="h-full w-full object-cover" />
            </div>
            ...
            <select className="flex h-10 w-full rounded-xl border border-input bg-white px-3 text-sm" {...form.register("style")}>
              {Object.entries(styleCategories).map(([category, keys]) => (
                <optgroup key={category} label={category}>
                  {keys.map((option) => (
                    <option key={option} value={option}>
                      {styleLabels[option as keyof typeof styleLabels]}
                    </option>
                  ))}
                </optgroup>
              ))}
