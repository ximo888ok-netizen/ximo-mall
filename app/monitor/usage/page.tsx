...
export default async function ApiUsageMonitorPage({
  ...
  const hours = Number(readSingle(resolvedSearchParams?.hours) ?? "24");
  const projectId = readSingle(resolvedSearchParams?.projectId) ?? "all";
  const category = readSingle(resolvedSearchParams?.category) ?? "all";
  const quotaState = readSingle(resolvedSearchParams?.quotaState) ?? "all";
  const success = readSingle(resolvedSearchParams?.success) ?? "all";
  const page = Number(readSingle(resolvedSearchParams?.page) ?? "1");
