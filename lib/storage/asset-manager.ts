...
export async function ensureStorageScaffold() {
  await Promise.all([
    ...
    ensureDir(path.join(rootDir(), "learning")),
  ]);
}
...
export async function saveUploadAsset(params: {
  ...
  await ensureDir(dir);
  const relativePath = path.join("uploads", params.projectId, safeName).replace(/\\/g, "/");
...
export async function saveGeneratedImage(params: {
  ...
  const relativePath = path.join("generated", params.projectId, params.sectionId, fileName).replace(/\\/g, "/");
...
export async function duplicateExportFile(params: {
  ...
  const relativePath = path.join("exports", params.projectId, safeName).replace(/\\/g, "/");
...
function learningDir(sessionId: string) {
  return path.join(rootDir(), "learning", sessionId);
}

export async function saveLearningImage(params: {
  sessionId: string;
  file: Buffer;
  fileName: string;
  mimeType: string;
  ...
  await ensureStorageScaffold();
  await ensureDir(learningDir(params.sessionId));

  const ext = extFromMime(params.mimeType) || "jpg";
  const safeName = `${Date.now()}-${nanoid(6)}.${ext}`;
  const relativePath = path.join("learning", params.sessionId, safeName).replace(/\\/g, "/");
  const absolutePath = path.join(rootDir(), relativePath);
...
export async function deleteLearningImage(relativePath: string) {
  try {
