...
export function detectModelCapabilities(modelId: string): CapabilityMap {
  ...
  if (/(gpt|gemini|claude|qwen|glm|deepseek|chat|instruct|command|llama|mistral|doubao)/.test(id)) {
    map.text = true;
    map.structured_output = true;
  }

  if (/(vision|vl|4o|omni|gemini|multimodal|qwen-vl|doubao-seed)/.test(id)) {
    map.vision = true;
    map.text = true;
    map.structured_output = true;
  }

  if (/(image|imagen|flux|sdxl|stable-diffusion|banana|nano-banana|recraft|seedream)/.test(id)) {
    map.image_gen = true;
    map.high_quality = true;
  }

  if (/(image.*edit|edit.*image|inpaint|mask|seededit|seedream)/.test(id)) {
    map.image_edit = true;
  }
  ...
  if (/qwen-image|imagen|flux(?!-.*text)|sdxl|stable-diffusion|banana|nano-banana|recraft|seedream|seededit/.test(id)) {
    map.text = false;
    map.structured_output = false;
    map.vision = false;
  }
