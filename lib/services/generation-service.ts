...
import { productAnalysisOutputSchema, type ProductAnalysisOutput } from "@/lib/ai/schemas/product-analysis";
import { prisma } from "@/lib/db/prisma";
import { getEnhancedGenerationConfig } from "@/lib/services/knowledge-sharing-service";
...
> = {
  "zh-CN": {
    ...
    footer: "商品详情模块 · ximo-mall AI 自动生成",
  },
  "en-US": {
    highlights: "Key Highlights",
    waitingForAsset: "Waiting for product image",
    footer: "Product Detail Section · Auto-generated by ximo-mall AI",
  },
  "ja-JP": {
    highlights: "主なポイント",
    waitingForAsset: "商品画像を待っています",
    footer: "商品詳細セクション · ximo-mall AI 自動生成",
  },
  "ko-KR": {
    highlights: "핵심 포인트",
    waitingForAsset: "상품 이미지를 기다리는 중",
    footer: "상품 상세 섹션 · ximo-mall AI 자동 생성",
  },
};
...
function getGenerationSettings(project: { modelSnapshot: unknown } | null) {
  ...
  const detailVisualAnchor = (snapshot.detailVisualAnchor as Record<string, string> | null) ?? null;
...
function buildSvgModelCandidates(provider: ProviderContext) {
  ...
  const candidates = [
    ...
    ...visionText
      .filter((item) => /gemini|gpt-4o|gpt-5|qwen|doubao|glm/i.test(item.modelId) && !/preview|experimental|beta|test/i.test(item.modelId))
      .map((item) => item.modelId),
...
function pickPrimaryProductAsset(projectAssets: AssetRecord[]) {
  // 只考虑用户上传的真实产品素材，排除 AI 生成的模块图和导出文件
  const productAssets = projectAssets.filter(
    (a) => a.type !== "GENERATED" && a.type !== "EXPORTED"
  );

  return (
    productAssets.find((asset) => asset.isMain) ??
    productAssets.find((asset) => asset.type === "MAIN") ??
    productAssets.find((asset) => asset.type === "PACKAGING") ??
    productAssets.find((asset) => ["ANGLE", "DETAIL", "REFERENCE"].includes(asset.type)) ??
    productAssets[0] ??
    null
  );
}
...
function mergeReferenceAssets(projectAssets: AssetRecord[], explicitReferenceAssets: AssetRecord[]) {
  ...
  const filteredExplicit = explicitReferenceAssets.filter(
    (a) => a.type !== "GENERATED" && a.type !== "EXPORTED"
  );
  const taggedAssets = projectAssets.filter(
    (a) => ["PACKAGING", "PRODUCT", "INGREDIENT", "INFO_CARD"].includes(a.type),
  );
  const merged = [primaryAsset, ...filteredExplicit, ...taggedAssets].filter(Boolean) as AssetRecord[];
...
/**
 * 根据模块类型重排参考图顺序：
 * - hero_01（主图）：PACKAGING 排最前
 * - hero_02~05（副图）：PRODUCT/INGREDIENT 排最前，PACKAGING 排最后
 * - 非头图：保持原顺序
 */
function reorderReferenceAssets(assets: AssetRecord[], section: { type: string; order: number }): AssetRecord[] {
  const taggedTypes = new Set(["PACKAGING", "PRODUCT", "INGREDIENT", "INFO_CARD"]);
  const hasTagged = assets.some((a) => taggedTypes.has(a.type));
  if (!hasTagged || section.type !== "HERO") return assets;

  const packaging = assets.filter((a) => a.type === "PACKAGING");
  const product = assets.filter((a) => a.type === "PRODUCT");
  const ingredient = assets.filter((a) => a.type === "INGREDIENT");
  const infoCard = assets.filter((a) => a.type === "INFO_CARD");
  const others = assets.filter((a) => !taggedTypes.has(a.type));

  if (section.order === 0) {
    // hero_01：外包装为主
    return [...packaging, ...product, ...ingredient, ...infoCard, ...others];
  }
  // hero_02~05：实物/调料为主，外包装排最后
  return [...product, ...ingredient, ...infoCard, ...others, ...packaging];
}
...
async function resolveReferenceAssets(referenceAssetIds: string[]) {
  ...
  const projectAssetIds = referenceAssetIds.filter((id) => !id.startsWith("library:"));
  const libraryAssetIds = referenceAssetIds
    .filter((id) => id.startsWith("library:"))
    .map((id) => id.slice("library:".length));

  const [projectAssets, libraryAssets] = await Promise.all([
    projectAssetIds.length > 0
      ? prisma.productAsset.findMany({
          where: { id: { in: projectAssetIds } },
        })
      : ([] as AssetRecord[]),
    libraryAssetIds.length > 0
      ? prisma.imageLibraryItem.findMany({
          where: { id: { in: libraryAssetIds } },
        })
      : ([] as Awaited<ReturnType<typeof prisma.imageLibraryItem.findMany>>),
  ]);

  const MAX_REFERENCE_IMAGES = 6;

  const convertedLibraryAssets = libraryAssets.map((item) => ({
    id: `library:${item.id}`,
    projectId: null,
  ...
  return [...projectAssets, ...convertedLibraryAssets].slice(0, MAX_REFERENCE_IMAGES);
}
...
function isFatalTextError(error: unknown) {
  if (!(error instanceof Error)) {
    return false;
  }

  return /monthly spending limit|spending limit|insufficient_quota|quota|billing|额度已用尽|月度限额|invalid token|unauthorized|forbidden|鉴权失败|invalid api key/i.test(
    error.message,
  );
}
...
async function generateSectionImageInternal(
  ...
  // 用户通过 Agent 指定的主图尺寸 → 宽高比映射
  let sectionAspectRatio = getSectionAspectRatio(section, generationSettings.imageAspectRatio);
  let heroOutputSize: string | undefined;
  if (section.type === "HERO" && options?.heroImageSize) {
    const sizeToRatio: Record<string, SectionImageAspectRatio> = {
      "1440x1440": "1:1",
      "800x800": "1:1",
      "750x750": "1:1",
      "750x1000": "3:4",
    };
    const overrideRatio = sizeToRatio[options.heroImageSize];
    if (overrideRatio) {
      sectionAspectRatio = overrideRatio;
      heroOutputSize = options.heroImageSize;
    }
  }

  const outputSize = heroOutputSize || getOutputSize(sectionAspectRatio);
  ...
  const effectiveReferenceAssets = reorderReferenceAssets(
  ...
  // 从分析结果中提取真实产品名称（作为产品身份的唯一锚点）
  const analysisResult = project.analysis?.normalizedResult as Record<string, unknown> | null;
  const realProductName = analysisResult?.productName as string | undefined;

  // 安全解析产品分析结果，用于生成层注入产品背景延伸约束
  let parsedAnalysis: ProductAnalysisOutput | undefined;
  if (analysisResult) {
    const parseResult = productAnalysisOutputSchema.safeParse(analysisResult);
  ...
  try {
    // 自定义模式下跳过知识增强，100% 按用户编辑内容生成
    let allReferenceImages = [...referenceImages];
    let promptEnhancement: string | null = null;

    if (!options?.customMode) {
      // 获取知识增强的配置
      const enhancedConfig = await getEnhancedGenerationConfig(
        projectId,
        sectionId,
        section.type,
        section.title
      ...
      // 合并参考图片（学习图片 + 项目素材）
      const learningReferenceImages = enhancedConfig.referenceImages || [];

      // 读取学习图片并添加到参考列表
      for (const ref of learningReferenceImages) {
        try {
          const buffer = await readStorageFile(ref.filePath);
          const dataUrl = `data:image/jpeg;base64,${buffer.toString("base64")}`;
          allReferenceImages.push(dataUrl);
        } catch {
          // 忽略读取失败的图片
        }
      }

      promptEnhancement = enhancedConfig.promptEnhancement || null;
    }

    let basePrompt: string;
    if (options?.regenerate) {
      basePrompt = buildRegenerationPrompt(
        section,
        ...
        project.platform,
        realProductName,
        section.type !== "HERO" ? generationSettings.detailVisualAnchor : null,
        project.style,
      ...
      basePrompt = buildSectionImagePrompt(
        section,
        effectiveReferenceAssets as ProductAsset[],
        sectionAspectRatio,
        generationSettings.contentLanguage,
    ...
    if (promptEnhancement) {
      prompt = `${basePrompt}\n\n风格参考：${promptEnhancement}`;
    }

    // 生成 negativePrompt（低密度风格需要专用负面约束）
    const lowDensityHero = section.type === "HERO" && !!project.style && 
      ["minimalist", "japanese_fresh", "healthy_light", "lifestyle_scene", "baby_parenting", 
       "ink_wash", "realistic_food_photo", "warm_homestyle", "vintage_chinese", "regional_memory"].includes(project.style);
    const negativePrompt = buildNegativePrompt(lowDensityHero);
...
export async function editSectionImage(
  ...
  // 从分析结果中提取真实产品名称
  const analysisResultEdit = project.analysis?.normalizedResult as Record<string, unknown> | null;
  const realProductNameEdit = analysisResultEdit?.productName as string | undefined;

  // 安全解析产品分析结果，用于生成层注入产品背景延伸约束
  let parsedAnalysisEdit: ProductAnalysisOutput | undefined;
  if (analysisResultEdit) {
    const parseResult = productAnalysisOutputSchema.safeParse(analysisResultEdit);
    if (parseResult.success) {
      parsedAnalysisEdit = parseResult.data;
    }
  }
...
export async function refineSectionImage(
  projectId: string,
  sectionId: string,
  options?: {
    preferredModelId?: string | null;
  ...
  if (!instruction) {
    throw new Error("请输入微调说明。");
  }

  // 从分析结果中提取真实产品名称
  const analysisResultRefine = project.analysis?.normalizedResult as Record<string, unknown> | null;
  const realProductNameRefine = analysisResultRefine?.productName as string | undefined;
  ...
  if (runningTask) {
    throw new Error("当前模块图仍在微调中，请等待这一轮完成后再试。");
  }
  ...
  try {
    const prompt = buildImageRefinePrompt(
      section,
      productReferenceAssets as ProductAsset[],
      instruction,
      sectionAspectRatio,
    ...
    try {
      ...
      const generation = await editWithFallback({
        ...
        projectId,
        sectionId,
        operation: "refine_section_image",
      });
      ...
      if (!generationSettings.allowSvgFallback) {
        const detail = error instanceof Error ? error.message : "Unknown image refine error";
        if (/monthly spending limit|spending limit|billing|quota/i.test(detail)) {
          throw new Error("当前 API Key 的图片生成额度已用尽。请前往代理商控制台提高或移除月度限额，或更换可用的 API Key。");
        }
        const summary = summarizeProviderImageFailure(detail, "edit");
      ...
      imageAsset = await saveGeneratedImage({
        ...
        metadata: {
          ...
          sourceReferenceAssetIds: productReferenceAssets.map((asset) => asset.id),
          primaryReferenceAssetId: productReferenceAssets[0]?.id ?? null,
          imageApiError: error instanceof Error ? error.message : "Unknown image refine api error",
        },
      });
    ...
    return { imageAsset, version, usedModel, generationMode, instruction };
  } catch (error) {
    await prisma.pageSection.update({
      where: { id: sectionId },
      data: {
    ...
    await failTask(task.id, error instanceof Error ? error.message : "Image refine failed");
    throw error;
  }
}
